...
def generate_rss_sz():
    try:
        print(f"Stahuji stránku Správy železnic: {URL}")
        ...
        # Kontejnery jednotlivých zpráv
        entries = soup.select(".szdc--aggregator-entry")
        print(f"Nalezeno {len(entries)} aktualit.")

        fg = FeedGenerator()
        fg.id(URL)
        fg.title('Správa železnic - DTM Technické standardy - Aktuality')
        fg.author({'name': 'SZ DTM Monitor'})
        fg.link(href=URL, rel='alternate')
        fg.description('Novinky z webu Správy železnic ohledně technických standardů DTM.')
        fg.language('cs')
        ...
        for entry in entries:
            ...
            pub_date = parse_cz_date(date_str)
            ...
            fe = fg.add_entry()
            fe.id(link or title) # Použijeme link jako ID
            fe.title(title)
            fe.link(href=link)
        ...
        print("Soubor feed_sz.xml byl úspěšně vytvořen.")

    except Exception as e:
        print(f"Nastal problém při generování SZ feedu: {e}")
